        ...
        if success:
            ...
            if "error" not in game_data:
                ...
                for team_name, team_data in teams.items():
                    ...
                    if stats:
                        print(
                            f"{team_name}: {stats.get('wins', 0)}-{stats.get('losses', 0)} ({stats.get('win_pct', 0):.3f})"
                        )
                ...
                if today_game:
                    print(
                        f"Today's game: {today_game.get('away_team')} @ {today_game.get('home_team')} - {today_game.get('status')}"
                    )
                else:
