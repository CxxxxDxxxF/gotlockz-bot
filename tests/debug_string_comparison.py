    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            ...
            if oakland:
                ...
                print(f"Lowercase comparison: '{api_name.lower()}' == '{search_name.lower()}' = {api_name.lower() == search_name.lower()}")
                
                # Check each character
                print("\nCharacter by character comparison:")
                for i, (api_char, search_char) in enumerate(zip(api_name, search_name)):
                    if api_char != search_char:
                        print(f"  Position {i}: API='{api_char}' (ord: {ord(api_char)}) vs Search='{search_char}' (ord: {ord(search_char)})")
                
                # Test the exact condition from the service
                condition = (oakland.get('sport', {}).get('id') == 1 and 
                           oakland.get('name', '').lower() == search_name.lower())
