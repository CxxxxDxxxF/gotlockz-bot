...
from bot.config import OPENAI_API_KEY, OPENAI_MODEL, ANALYSIS_TEMPERATURE
from bot.utils.data_enrichment import enrich_bet_analysis
...
async def analyze_bet_slip(
    ...
    try:
        logger.info(
            f"Starting AI analysis for bet type: {bet_details.get('type', 'unknown')}")
...
def _build_analysis_prompt(
    ...
    if 'game_info' in enriched_data:
        ...
        if 'weather' in game_info:
            ...
            prompt_parts.append(
                f"- Weather: {weather.get('temperature', 'Unknown')}, {weather.get('conditions', 'Unknown')}")
            prompt_parts.append(
                f"- Wind: {weather.get('wind_speed', 'Unknown')} {weather.get('wind_direction', 'Unknown')}")

        if 'starting_pitchers' in game_info:
            pitchers = game_info['starting_pitchers']
            prompt_parts.append(
                f"- Pitchers: {pitchers.get('team1_pitcher', 'TBD')} vs {pitchers.get('team2_pitcher', 'TBD')}")
    ...
    if 'h2h_stats' in enriched_data:
        ...
        prompt_parts.append(
            f"- {enriched_data.get('away', 'Team1')} wins: {h2h.get('team1_wins', 0)}")
        prompt_parts.append(
            f"- {enriched_data.get('home', 'Team2')} wins: {h2h.get('team2_wins', 0)}")
        prompt_parts.append(
            f"- Avg runs: {h2h.get('avg_runs_team1', 0)} vs {h2h.get('avg_runs_team2', 0)}")
    ...
    if 'player_stats' in enriched_data:
        ...
        prompt_parts.append(f"\nPLAYER STATISTICS:")
        prompt_parts.append(
            f"- Player: {enriched_data.get('player', 'Unknown')}")
        # Add relevant stats based on prop type
        if 'AVG' in player_stats:
            prompt_parts.append(
                f"- Season AVG: {player_stats.get('AVG', 'Unknown')}")
        if 'HR' in player_stats:
            prompt_parts.append(
                f"- Season HR: {player_stats.get('HR', 'Unknown')}")
    ...
    if 'edge_analysis' in enriched_data:
        ...
        prompt_parts.append(f"\nEDGE ANALYSIS:")
        prompt_parts.append(
            f"- Implied Probability: {edge.get('implied_probability', 0):.3f}")
        prompt_parts.append(
            f"- True Probability: {edge.get('true_probability', 0):.3f}")
...
async def generate_pick_summary(
    ...
    summary_parts.append(
        f"🔒 **GOTLOCKZ {pick_type.upper()} PICK #{pick_number}** 🔒")
    summary_parts.append("")
    ...
    if 'recommendation' in analysis:
        ...
        summary_parts.append(
            f"**Recommendation:** {rec.get('action', 'Unknown')}")

        if 'stake_suggestion' in rec:
            summary_parts.append(
                f"**Stake:** {rec.get('stake_suggestion', 'Unknown')}")
