...
class Jarvis:
    ...
    def check_alerts(self, stats: Dict) -> List[str]:
        ...
        if stats.get('temperature_c'):
            ...
            if temp > 85:
                alerts.append(
                    f"🚨 CRITICAL: CPU temperature is {temp}°C - System overheating!")
            elif temp > 75:
                alerts.append(
                    f"⚠️  WARNING: CPU temperature is {temp}°C - Monitor closely")
        ...
        if stats['memory_percent'] > 90:
            alerts.append(
                f"🚨 CRITICAL: Memory usage is {stats['memory_percent']}%")
        elif stats['memory_percent'] > 80:
            alerts.append(
                f"⚠️  WARNING: Memory usage is {stats['memory_percent']}%")

        # Disk alerts
        if stats['disk_percent'] > 90:
            alerts.append(
                f"🚨 CRITICAL: Disk usage is {stats['disk_percent']}%")
        elif stats['disk_percent'] > 80:
            alerts.append(
                f"⚠️  WARNING: Disk usage is {stats['disk_percent']}%")
        ...
        if stats.get('battery_percent') is not None:
            ...
            if battery < 10:
                alerts.append(
                    f"🔋 CRITICAL: Battery at {battery}% - Connect charger immediately!")
            elif battery < 20:
                alerts.append(
                    f"🔋 WARNING: Battery at {battery}% - Consider charging")
    ...
    def optimize_system(self):
        ...
        for proc in high_cpu_processes:
            if proc['cpu_percent'] > 50 and proc['name'] not in [
                    'kernel_task', 'WindowServer']:
                try:
                    os.kill(proc['pid'], 15)  # SIGTERM
                    optimizations.append(
                        f"✅ Terminated high-CPU process: {proc['name']}")
                except BaseException:
                    pass
        ...
        try:
            subprocess.run(
                ['sudo', 'pmset', '-a', 'lowpowermode', '1'], capture_output=True)
    ...
    def monitor_system(self, interval: int = 30):
        ...
        while self.monitoring:
            try:
                ...
                print(f"{'='*60}")
                print(
                    f"CPU: {stats['cpu_percent']}% | Memory: {stats['memory_percent']}% | Disk: {stats['disk_percent']}%")
                if stats.get('temperature_c'):
                ...
                if alerts:
                    ...
                    for alert in alerts:
                        print(f"  {alert}")
                        self.speak(
                            alert.split(':')[1] if ':' in alert else alert)
                ...
                for i, proc in enumerate(top_procs, 1):
                    print(
                        f"  {i}. {proc['name']} (PID: {proc['pid']}) - CPU: {proc['cpu_percent']}%")
        ...
        while self.running:
            try:
                ...
                if command == 'quit' or command == 'exit':
                    ...
                    for i, proc in enumerate(top_procs, 1):
                        print(
                            f"  {i}. {proc['name']} (PID: {proc['pid']}) - CPU: {proc['cpu_percent']}% | Memory: {proc['memory_percent']}%")
...
def main():
    parser = argparse.ArgumentParser(
        description='JARVIS - System Management Assistant')
    parser.add_argument(
        '--monitor',
        action='store_true',
